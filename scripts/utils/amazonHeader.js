...
export let previousSearches = JSON.parse(localStorage.getItem('previousSearches')) || []
...
export function setUpSearchBar(searchBar, searchSuggestion, previousSearchList, fun) {
  searchBar.addEventListener('input', () => {
    previousSearchList.innerHTML = renderPreviousSearches(filterPreviousSearches(searchBar.value))
    setUpPrevSearchButton(previousSearchList, fun, searchSuggestion, searchBar)
  });
...
function setUpPrevSearchButton(previousSearchList, fun, searchSuggestion, searchBar) {
  searchSuggestion.querySelectorAll('.js-previous-search-button').forEach((button) => {
    button.addEventListener('click', () => {
      autofill(searchBar, previousSearchList, button.dataset.searchValue);
      searchSuggestion.classList.remove('show-search-suggestion');
      if (location.href.includes('amazon.html')) {
        fun();
      }else{
  ...
  searchSuggestion.querySelectorAll('.js-remove-previous-search').forEach((button) => {
    button.addEventListener('click', () => {
...
function removeFromPreviousSearch(value) {
  return previousSearches.filter((prevSearchValue) => prevSearchValue !== value);
}